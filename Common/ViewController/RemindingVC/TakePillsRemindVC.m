...
@interface TakePillsRemindVC ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation TakePillsRemindVC
...
- (void)initNavigationView {
    ...
    label.text= NSLocalizedString(@"吃药提醒",nil);
...
- (NSMutableArray *)mutDataArr {
    if (!_mutDataArr) {
        ...
        _mutDataArr = [NSMutableArray arrayWithArray:@[@[NSLocalizedString(@"间隔时间",nil)],@[NSLocalizedString(@"开始时间",nil)],@[NSLocalizedString(@"结束时间",nil)]]];
    }
    return _mutDataArr;
}
...
- (NSMutableArray *)mutDetailDataArr {
    if (!_mutDetailDataArr) {
        NSMutableArray *tempArr = [[NSUserDefaults standardUserDefaults] objectForKey:@"kNedication"];
        if (tempArr) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *KCellTakePillID = @"KCellTakePillID";
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:KCellTakePillID];
    if (cell == nil) {
        cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:KCellTakePillID];
    }
