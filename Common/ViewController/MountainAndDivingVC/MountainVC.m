...
- (void) initialNavigationbar
{
    ...
    self.title = NSLocalizedString(@"登山", nil);
...
- (void)createTopInfoViews {
    ...
    NSArray *testArr1 = @[@"cloudy day", @"6:00", @"99℃", @"25:00"];
    NSArray *testArr2 = @[@"Current weather", @"Sunrise time", @"Current temperature", @"Sunset time"];
    
    NSArray *languages = [NSLocale preferredLanguages];
    NSString *currentLanguage = [languages objectAtIndex:0];
    
    if ([currentLanguage isEqualToString:@"zh-Hans-US"]||[currentLanguage isEqualToString:@"zh-Hans"]) {
        testArr1 = @[@"阴天", @"6:00", @"99℃", @"25:00"];
        testArr2 = @[@"当前天气", @"日出时间", @"当前温度", @"日落时间"];
    }
...
-(void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx {
    ...
    if ([layer.name isEqualToString:@"humingminTop"]) {
        ...
        CGContextAddEllipseInRect(ctx, CGRectMake(0, 40, kScreenWidth, 1.5));
        ...
        _myTempPoint = CGPointMake(5 * myTempView2.frame.size.width / 6  - 2, 7 * myTempView2.frame.size.height / 8.0);
        ...
        for (NSInteger i = 0; i < 100; i ++) {
            ...
            int someHeight = arc4random_uniform(120);
//            CGContextAddRect(ctx,CGRectMake(5 * myTempView2.frame.size.width / 6  - 2 * i, 7 * myTempView2.frame.size.height / 8.0  - someHeight, 1, someHeight));
//            CGContextDrawPath(ctx, kCGPathFillStroke);

            CGContextAddLineToPoint(ctx, 5 * myTempView2.frame.size.width / 6  - 10 * i, 7 * myTempView2.frame.size.height / 8.0  - someHeight);
            CGContextStrokePath(ctx);
            
            _myTempPoint = CGPointMake(5 * myTempView2.frame.size.width / 6  - 10 * i, 7 * myTempView2.frame.size.height / 8.0  - someHeight);
        }
    }
...
- (void)createChartInfo {
    ...
    _myLayerOfChart.shadowOpacity = 0.85;
...
- (void)createOtherViews {
    ...
    myShenduLabel.text = NSLocalizedString(@"登山数据", nil);
    ...
    UILabel *myLabelOfSomeInfo = [[UILabel alloc] initWithFrame:CGRectMake(0 + 40, myTempView2.frame.size.height + myTempView2.frame.origin.y + 8 + 7 + 17, kScreenWidth - 80, 40)];
    ...
    myLabelOfSomeInfo.text = @"Keep the speed,then you will get the target at 12:00";
    
    NSArray *languages = [NSLocale preferredLanguages];
    NSString *currentLanguage = [languages objectAtIndex:0];
    
    if ([currentLanguage isEqualToString:@"zh-Hans-US"]||[currentLanguage isEqualToString:@"zh-Hans"]) {
        myLabelOfSomeInfo.text = @"保持当前速度，在12：00分时，可以达到目标高度";
    }
    ...
    if (kScreenHeight == 480) {
        ...
        myLabelOfSomeInfo.transform = CGAffineTransformTranslate(CGAffineTransformMakeScale(1.0f, 1.0f), 0, -15 - 25);
    } else if (kScreenHeight == 568) {
...
- (void)createDetailInfoDisplay {
    ...
    NSArray *arrOfType = @[@{@"ing_mountaineering":@"Mountaineering goal"}, @{@"ing_altitude":@"Current elevation"}, @{@"ing_time":@"Total time"}];
    ...
    if ([currentLanguage isEqualToString:@"zh-Hans-US"]||[currentLanguage isEqualToString:@"zh-Hans"]) {
        arrOfType = @[@{@"ing_mountaineering":@"登山目标"}, @{@"ing_altitude":@"当前海拔"}, @{@"ing_time":@"所用时间"}];
    }
