...
- (id)init
{
    ...
    if (self) {
        ...
        _headerView.myretrunBlockDate = ^(NSString *dateStr) {
            NSLog(@"%@", dateStr);
        };
...
//        _table.separatorColor = [UIColor clearColor];
        UITapGestureRecognizer *tagTable = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapTableView:)];
        [_table addGestureRecognizer:tagTable];
...
#warning aaa
    CGRect headerFrame = CGRectMake(0, 0, width, 44 + 20);
...
- (void)_layoutCellsAnimated:(BOOL)animated
{
    ...
    for (NSUInteger row = 0; row < rowCount; row++) {
        for (NSUInteger column = 0; column < columnCount; column++) {
            ...
            __block NSInteger anum = 0;
            cell.myReturnBlockOfIndex = ^(NSInteger num) {
                aindex = num;
                weakSelf.index = num;
            };
            cell.myReturnBlockOfNum = ^(NSInteger num) {
                anum = num;
                weakSelf.num = num;
            };
...
//            self.mytturnBlockOfIndexNext(self.index);
//            self.myReturnBlockOfNumNext(self.num);
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    self.returnClickBlock(self.index, self.num); // 😄
}
