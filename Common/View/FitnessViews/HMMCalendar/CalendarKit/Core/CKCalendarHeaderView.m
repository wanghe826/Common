...
- (void)layoutSubviews
{
    ...
    _forwardButton = [[MBPolygonView alloc] initWithFrame:forwardFrame numberOfSides:3 andRotation:90.0 andScale:6.5];
    _backwardButton = [[MBPolygonView alloc] initWithFrame:backFrame numberOfSides:3 andRotation:30.0 andScale:6.5];
...
- (UILabel *)_columnLabelWithTitle:(NSString *)title
{
    ...
    [l setFont:[UIFont boldSystemFontOfSize:12]];
    [l setShadowOffset:CGSizeMake(0, 0)];
    [l setText:title];
...
- (void)forwardButtonTapped
{
    ...
    self.myretrunBlockDate(self.monthTitle.text);
//    NSLog(@"%@", self.monthTitle.text);
}
