...
//        _inactiveSelectedBackgroundColor = kCalendarColorDarkGray;
        _inactiveSelectedBackgroundColor = [UIColor redColor];
...
//        _cellBorderColor = kCalendarColorCellBorder;
        _cellBorderColor = [UIColor whiteColor];
...
//    UIView *myTestView = [[UIView alloc] initWithFrame:self.bounds];
//    myTestView.backgroundColor = kRandomColor(1.0f);
...
//    self.index
//    self.number
    self.myReturnBlockOfIndex(self.index);
    self.myReturnBlockOfNum([self.number integerValue]);
}
