...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_isSportVC) {
        return (self.frame.size.width) / (24);
    } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    int randowNum;
    if (_isSportVC) {
        testArr = @[HexRGBAlpha(0x19bc9d, 0.6)];
        randowNum = 0;
        cell.cellColor = testArr[randowNum];
        CGFloat testArrHeight = self.frame.size.width;
        cell.cellHeight = arc4random_uniform((int)testArrHeight);
        cell.isSportVC = YES;
    } else {
        testArr = @[HexRGBAlpha(0x19bc9d, 0.6), HexRGBAlpha(0x0687, 0.6), HexRGBAlpha(0xbfa875, 0.6)];
        randowNum = arc4random_uniform(3);
        cell.cellColor = testArr[randowNum];
